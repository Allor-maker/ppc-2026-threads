...
SmyshlaevAConjGradTaskALL::SmyshlaevAConjGradTaskALL(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = 0;
}

bool SmyshlaevAConjGradTaskALL::ValidationImpl() {
  return (GetInput() > 0) && (GetOutput() == 0);
}

bool SmyshlaevAConjGradTaskALL::PreProcessingImpl() {
  GetOutput() = 2 * GetInput();
  return GetOutput() > 0;
}

bool SmyshlaevAConjGradTaskALL::RunImpl() {
...
bool SmyshlaevAConjGradTaskALL::PostProcessingImpl() {
  GetOutput() -= GetInput();
  return GetOutput() > 0;
}
